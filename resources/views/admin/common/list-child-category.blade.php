...
<div id="js-sortable{{$parent_id}}" class="p-hide__category is-show__hide--child{{$parent_id}}">
    ...
    <div class="p{{$parent_id}}">
        <dl class="o-dl__display--table parent" data-sort="{{ $category->id }}">
            ...
            <dd class="o-dtDd__display--table-cell text-left ol-{{$level}}">{{$category->name ?? ''}}</dd>
            ...
            <dd class="o-dtDd__display--table-cell o-iconRight pr-3 o-dt__width--5"><a href="{{ route('admin.category.detail', ['id' => $category->id])}}"><i class="fas fa-pencil-alt o-fontSize_1-5 pr-5"></i></a><i data-action="{{ route('admin.category.delete')}}" data-id="{{ $category->id}}" class="far fa-times fa-2x js-delete-custom--ajax"></i></dd>
        </dl>
        @if (count($category->childs))
        @include('admin.common.list-child-category', ['categories' => $category->childs, 'parent_id' => $category->id, 'level' => $level + 1])
        @endif
    </div>
    @endforeach
</div>